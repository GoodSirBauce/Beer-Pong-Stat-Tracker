...
def create_players_listbox(window, display_stats):
    ...
    players_listbox.grid(row=0, column=0, padx=10, pady=10, sticky=tk.N + tk.S)
...
def create_teams_listbox(window, display_stats):
    ...
    teams_listbox.grid(row=0, column=1, padx=10, pady=10, sticky=tk.N + tk.S)
...
def show_listbox(window, item_type):
    # Remove existing listboxes in the window
...
def display_stats(window, selected_item, item_type):
    ...
    stats_frame.grid(row=0, column=2, padx=10, pady=10, sticky=tk.N)

    # Display the statistics in a formatted way
    for index, (stat_name, stat_value) in enumerate(stats_data.items()):
        label = tk.Label(stats_frame, text=f"{stat_name}: {stat_value}")
        label.pack(anchor=tk.W)  # Use pack() instead of grid()
